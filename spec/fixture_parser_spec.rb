describe FixToChix::FixtureParser do
  ...
  describe "parsing" do
    ...
    it "writes defines factories" do
      @parser.parse_fixture
      @parser.output_buffer.should match(/Factory.define :anakin, :class_name => Author/)

      parser = FixToChix::FixtureParser.new(File.dirname(__FILE__) + "/fixtures/posts.yml")
      parser.parse_fixture
      parser.output_buffer.should match(/Factory.define :hello_world, :class_name => Post do \|p\|/)
    end

    it "write attributes " do
      @parser.parse_fixture
      @parser.output_buffer.should match(/a.blog/)

      parser = FixToChix::FixtureParser.new(File.dirname(__FILE__) + "/fixtures/posts.yml")
      parser.parse_fixture
      parser.output_buffer.should match(/p.title/)

    end

    it "writes attributes with respective values" do
      @parser.parse_fixture
      @parser.output_buffer.should match(/a.blog 'I write a blog'/)
      @parser.output_buffer.should match(/a.age 900/)
    end
